 ...
 * @property {GetOrApplySync} getOrApplySync
 */
/**
...
export function makeCapTP(ourId, rawSend, bootstrapObj = undefined, opts = {}) {
  const {
    ...
    epoch = 0,
    getOrApplySync: rawGetOrApplySync,
  } = opts;
  ...
  if (rawGetOrApplySync) {
    /** @type {GetOrApplySync} */
    const getOrApplySync = (target, prop, methodArgs = undefined) => {
      assert(
      ...
      assert(
        rawGetOrApplySync,
        X`Sync(${target}) failed; no opts.getOrApplySync supplied to makeCapTP`,
      );
      return rawGetOrApplySync(target, prop, methodArgs);
    };

    addSync.Sync = makeSync(getOrApplySync);
  }
