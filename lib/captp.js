...
export function makeCapTP(ourId, rawSend, bootstrapObj = undefined, opts = {}) {
  ...
  const answerToTrapNextBuffer = new Map();
  /** @type {Map<number, any>} */
  ...
  const handler = {
      ...
      if (trap) {
        try {
          ...
          assert.typeof(
            giveTrapReply,
            'function',
            X`CapTP internal error; cannot answer Trap(x) without opts.giveTrapReply`,
          );
        } catch (e) {
          sendReturn(true, e);
          throw e;
        ...
        sendReturn = async (isReject, value) => {
          ...
          // Allow the caller to iterate via `CTP_TRAP_NEXT_BUFFER` messages.
          ...
          const nextInSequence = async (seq, data) => {
            ...
            assert.equal(
              seq,
              nextSeq,
              X`Invalid CTP_TRAP_NEXT_BUFFER; seq ${seq} is not ${nextSeq}`,
            );
            nextSeq += 1;
            ...
            if (status.done) {
              ...
              answerToTrapNextBuffer.delete(questionID);
            }
          };
          answerToTrapNextBuffer.set(questionID, nextInSequence);
    ...
    async CTP_TRAP_NEXT_BUFFER(obj) {
      const { questionID, data, seq } = obj;
      const nextInSequence = answerToTrapNextBuffer.get(questionID);
  ...
  if (takeTrapReply) {
    ...
    const makeTrapImpl = implMethod => (target, ...implArgs) => {
      ...
      const it = takeTrapReply(implMethod, slot, implArgs);
      const nextTrap = prep => {
