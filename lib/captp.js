...
const STARTING_SEQUENCE_NUMBER = 23;
 ...
 * @property {TakeSyncReply} takeSyncReply if specified, enable this CapTP end
 * to use Sync(target) to block while the recipient resolves and communicates
 * the message
...
export function makeCapTP(ourId, rawSend, bootstrapObj = undefined, opts = {}) {
  ...
  const syncableExported = new WeakSet();
  ...
  function convertValToSlot(val) {
    if (!valToSlot.has(val)) {
      ...
      if (isPromise(val)) {
        ...
        // 'o+' for normal objects, or `s+` for syncable.
        lastExportID += 1;
        const exportID = lastExportID;
        if (syncableExported.has(val)) {
          slot = `s+${exportID}`;
        } else {
          slot = `o+${exportID}`;
        }
  ...
  const handler = {
    ...
    async CTP_CALL(obj) {
      ...
      const { questionID, target, sync } = obj;
      ...
      if (sync) {
        try {
          assert(
            syncableExported.has(val),
            X`Refused Sync(${val}) because target has not been exported as syncable`,
          );
          assert.typeof(
            giveSyncReply,
            'function',
            X`CapTP internal error; cannot make synchronous answer without opts.giveSyncReply`,
          );
        } catch (e) {
          sendReturn(true, e);
          throw e;
        ...
        sendReturn = async (isReject, value) => {
          // sendSync may return an iterator.
          const serialized = serialize(harden(value));
          const it = giveSyncReply(isReject, serialized);

          // Allow the caller to iterate via `CTP_NEXT` messages.
          let nextSeq = STARTING_SEQUENCE_NUMBER;
          let pendingP;
          const nextInSequence = async (seq, data) => {
            ...
            assert.equal(
              seq,
              nextSeq,
              X`Invalid CTP_NEXT; seq ${seq} is not ${nextSeq}`,
            );
            nextSeq += 1;
            pendingP = it.next(data);
            const status = await pendingP;
            if (status.done) {
              // No more "nexts" are required.
              answerNext.delete(questionID);
            }
          };
          answerNext.set(questionID, nextInSequence);

          // Prime the pump with the first iteration.
          nextInSequence(nextSeq, undefined);
        };
      }
    ...
    // Send the next part of a "sync" call's result.
    async CTP_NEXT(obj) {
      const { questionID, data, seq } = obj;
      const nextInSequence = answerNext.get(questionID);
  ...
  const exportAsSyncable = obj => {
    assert(
      getInterfaceOf(obj) !== undefined,
      X`exportAsSyncable(${obj}) argument must be Remotable`,
    );
    syncableExported.add(obj);
    return obj;
  };
  ...
  const rets = {
    ...
    unserialize,
    exportAsSyncable,
    Sync: /** @type {Sync | undefined} */ (undefined),
  };

  if (takeSyncReply) {
    // Create the Sync proxy maker.
    const makeSyncImpl = implMethod => (target, ...implArgs) => {
      assert(
      ...
      assert(
        slot && slot[1] === '-',
        X`Sync(${target}) target was not imported`,
      );
      assert(
        slot[0] === 's',
      ...
      assert(
        takeSyncReply,
        X`Sync(${target}) failed; no opts.takeSyncReply supplied to makeCapTP`,
      );
      ...
      // Set up the sync call with its identifying information.
      ...
      let firstTime = true;
      let status = nextSync(firstTime);
      let seq = STARTING_SEQUENCE_NUMBER;
      while (!status.done) {
        if (firstTime) {
          ...
          send({
            type: 'CTP_CALL',
            epoch,
            sync: true, // This is the magic marker.
            questionID,
            target: slot,
            method,
          });
 ...
 * exportAsSyncable<T>(x: T): T, Sync: Sync }}
 */
export function makeLoopback(ourId, opts = {}) {
  ...
  const {
    ...
    *takeSyncReply(implMethod, slot, implArgs) {
      let value;
      ...
      try {
        ...
        value = nearSyncImpl[implMethod](far, implArgs[0], implArgs[1]);
      } catch (e) {
        isException = true;
        value = e;
      }
  ...
  const {
    ...
    giveSyncReply(_isReject, _serialized) {
      throw Error(`makeLoopback giveSyncReply is not expected to be called`);
    },
  });
