...
export function makeCapTP(ourId, send, bootstrapObj = undefined) {
  ...
  function convertSlotToVal(theirSlot) {
    ...
    // (??? cwebber): Why is this inverted this way?  In what circumstance
    //   would the other side have the direction be '-', and what does that
    //   mean?
  ...
  const handler = {
    ...
    CTP_CALL(obj) {
      ...
      // (??? cwebber): Not sure that's well phrased or even if I understand
      //   right.
