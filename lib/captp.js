  ...
  const handler = {
    ...
    async CTP_RETURN(obj) {
      ...
      if (!questions.has(answerID)) {
        throw new Error('Got an answer to a question we have not asked. (answerID = '.concat(answerID, ' )'));
      }
    ...
    async CTP_RESOLVE(obj) {
      ...
      if (!imports.has(promiseID)) {
        throw new Error('Got a resolvement of a promise we have not imported. (promiseID = '.concat(promiseID, ' )'));
      }
