...
- [ ] so now we need to chase more label passing up the chain from the `'host'` and `'guest'` labels in `packages/daemon/src/daemon.js`
- [ ] the 3rd worker above that is still `<untitled>` is from @endo/lal's setup, so whatever it's formulating, isn't getting any label
