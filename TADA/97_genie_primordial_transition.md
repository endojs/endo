...
- `activatePiAgent` extracted in `packages/genie/main.js`; shared
  between the cold-boot piAgent path and the primordial → piAgent
  hand-off.
- `runAgentLoop` now reads the agent pack lazily off
  `state.{piAgent, heartbeatAgent, observer, reflector, genieTools,
  pendingHeartbeatTicks}`, which `activatePiAgent` populates in
  place.  `makeBuiltinSpecials` was updated to read `agents.X`
  lazily for the same reason — the dispatcher built in primordial
  mode picks up the new pack the moment activation completes.
- Primordial-mode `/model commit` now yields
  `Configuration saved …` → `Activating piAgent…` → `Ready: …`
  chunks via the model-handler, while the worker logs
  `[genie:<name>] Transitioned to piAgent mode (model: <p>/<m>)`
  followed by the backwards-compatible
  `[genie:<name>] agent ready (model: …)` line.
- piAgent-mode `/model commit` calls `state.requestRestart`, which
  schedules `process.exit(0)` ~200 ms after the chunks flush so
  the daemon reincarnates the worker with the freshly-persisted
  config.
- Activation failures roll back the persisted config via
  `clearPersistedConfig` so a subsequent worker boot does not load
  a config the activation could not honour.  Race-guarded with a
  one-shot `makePromiseKit` so concurrent `/model commit` calls
  await the same activation promise.
- Tests: model-handler.test.js exercises the new commit flow and
  `requestRestart` side-effect; self-boot.test.js gains two
  integration tests covering the in-flight hand-off and the
  restart-replays-config path.
- Acceptance suite (`npx ava test/boot/ test/primordial/
  test/loop/ --timeout=120s`) passes 122 tests, including the
  Phase-2 integration cases.
