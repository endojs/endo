...
| **Content hash** | `EndoBlob` exposed both `sha256()` (hex) and `getInfo().hash` (base64) — two encodings of one digest on one cap | **`EndoBlob.sha256()` removed.** The daemon never read the hash off a cap (it always holds the hex from `contentStore.store()` / the formula), so the method was a remote-only accessor now served by `getInfo().hash` (base64). `EndoBlob` collapses to `ReadableBlobRangeInterface`. `SnapshotBlob` / `SnapshotTree` / `EndoReadableTree` keep `sha256()` (hex) — they have no `getInfo`, so it is their only identity accessor. The hex store address stays internal; tests convert base64→hex at the path-building callsite (`@endo/hex` + `@endo/base64`). |
