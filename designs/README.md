...
*Last updated: 2026-07-18 (reframed [ocapn-noise-key-only-session-boundary](ocapn-noise-key-only-session-boundary.md) per maintainer direction to a **dumb key-sniffing ciphertext relay**: the relay/mux forwards the untouched encrypted session to the true OCapN listener by the plaintext responder key and does not terminate or depend on Noise, so Noise termination, authentication, and decryption stay at the recipient; splits into a Noise-free relay package plus a terminating listener package with a static, `SIGHUP`-reloadable route mapping, resolves both prior open questions, and adds a Node-prototype/Rust-data-plane (bespoke CBOR controller protocol adapted to Exo interfaces on the JS facade) plan. Summary, dependency graph, M4, and estimate rows synced.
...
| [ocapn-noise-key-only-session-boundary](ocapn-noise-key-only-session-boundary.md) | 2026-07-18 | 2026-07-18 | Proposed |
...
| ocapn-noise-key-only-session-boundary | Proposed | Dumb key-sniffing ciphertext relay: forwards the encrypted session to the true OCapN listener by the plaintext responder key; Noise termination stays at the recipient; splits into a Noise-free relay package plus a terminating listener package with a static SIGHUP-reloadable route mapping |
...
| ocapn-noise-key-only-session-boundary | M | 3-4 days | 4 | Noise-free key-sniffing relay package plus terminating listener package, OCapN adapters, static SIGHUP-reloadable per-daemon forwarding mapping, and WebSocket-to-relay handoff migration; Node prototype prepared for a Rust data plane behind a CBOR controller protocol |
