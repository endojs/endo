...
`pc-agent-tools-and-agentry-designs` pair. `@endo/agent-tools` specifies
...
eval-vs-optimize distinction the git code-mode eval harness draws),
...
| [agentry-agent-builder](agentry-agent-builder.md) | 2026-06-03 | 2026-06-25 | Not Started |
...
| agentry-agent-builder | Not Started | `@endo/agentry` `defineAgent`: the agent builder that lets someone build their own lal (dogfooded by reconstructing lal itself). A `defineAgent` (powerless template) plus `makeAgent` (instance with powers) pair, a new module in #308's optimizer and eval package, following the exo `define*`/`make*` factorization. Primary interaction mode is code-mode `execute` over petname-bound endowments (not a multi-interface cli/sdk/web export); discrete `arg0`-style tools are kept as a distinct second mode. Code-mode result rendering uses lal's real SmallCaps marshaller (`packages/lal/agent.js`), not `JSON.stringify`. `defineAgent` carries method guards, wire schemas, attenuation policies (functions-of-cap), prompts, compaction selector, `prepareArguments`; `makeAgent(template, powers)` binds workspace `Filesystem`, `Spawner`, authToken, and `(fs,crypto,url)` confinement powers per instance. No `harness` abstraction (one confined pi loop via `@earendil-works/pi-agent-core` v0.78.0, loaded into a fresh Endo Compartment per #297 at make time); composes tool selection (`@endo/agent-tools` include groups) plus attenuation policies (genie's read-only/subtree/git-only/exec registry) plus wire schemas (`Tool.parameters` plus MCP `inputSchema`, filled at define time) plus `compaction` (pi-default or genie's observer/reflector pair). The symmetric SmallCaps Hilbert-Hotel decode (`coerceBigintArgs` plus `unescapeHilbertHotel` plus LLM-JSON fixups) runs in pi's `prepareArguments` to deflect bigints and type-introducer string collisions. First pass reconstructs lal plus genie via their own `define(Lal\|Genie)Agent` plus `make(Lal\|Genie)Agent` pairs; fae pair is deprioritized (fae keeps its own loop, not based on pi for now). Capped at the eval-vs-optimize distinction the git code-mode eval harness draws. Drives the #404 wizard's Submit (which calls `makeAgent`); wires the `Filesystem` tools plus Git adapters into the #370 loop |
