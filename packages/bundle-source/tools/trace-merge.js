...
const main = async () => {
  ...
  const top = toInt(stringOption(topRaw, '--top'));
  ...
  await fs.writeFile(
    stringOption(outTrace, '--out-trace'),
    JSON.stringify(
  ...
  await fs.writeFile(
    stringOption(outSummary, '--out-summary'),
    JSON.stringify(summary, null, 2),
  );
  await fs.writeFile(
    stringOption(outMarkdown, '--out-markdown'),
    summarizeMarkdown(summaryTop, focusSpans),
  );
