...
test('recoverable syntax errors throw AggregateError when onParseError is not provided', t => {
  const fixtureUrl = new URL('./fixture/recoverable-error.js', import.meta.url);
  const fixturePath = fileURLToPath(fixtureUrl);
  const fixtureBytes = readFileSync(fixturePath);
  const fixtureFileUrl = fixtureUrl.href;
  ...
  // recoverable parse errors by throwing, since a module with any parse
  // errors will fail to execute at runtime anyway.
  const { sync } = createParsers();
