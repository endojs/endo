...
const mergePerLanguageConfig = (
   ...
   */
  const unlessEmpty = obj => {
    return fromEntries(entries(obj).filter(([_, v]) => v !== undefined));
  };
  ...
  return {
    ...
    ...unlessEmpty({
      onModuleComplete: langOnModuleComplete ?? sharedOnModuleComplete,
      onModuleStart: langOnModuleStart ?? sharedOnModuleStart,
      onParseError: langOnParseError ?? sharedOnParseError,
      babelParserOptions: langBabelParserOptions ?? sharedBabelParserOptions,
      babelGeneratorOptions:
        langBabelGeneratorOptions ?? sharedBabelGeneratorOptions,
      log: langLog ?? sharedLog,
