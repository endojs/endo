...
import { analyzeCjs, analyzeModule } from '@endo/module-source/analyzer.js';
...
const stripTypes = (source, location) => {
  if (
    !('stripTypeScriptTypes' in nodeModule) ||
    typeof nodeModule.stripTypeScriptTypes !== 'function'
  ) {
    throw new Error(
      `Cannot strip TypeScript types from ${location}; requires Node.js v22.13.0 / v23.2.0 or newer.`,
    );
  }
