    ...
    if (errors?.length) {
      if (typeof onParseError === 'function') {
        ...
        throw new AggregateError(
          errors,
          `Failed to parse module at ${location}`,
        );
      }
    }
...
const createAsyncParserFromConfig = (language, config, pool) => {
  ...
  const parse = async (
    ...
    const { bytes: transformedBytes, visitorResults, parseErrors } = message;
    ...
    if (parseErrors?.length) {
      if (typeof onParseError === 'function') {
        onParseError(parseErrors, location);
      } else {
        throw new AggregateError(
          parseErrors,
          `Failed to parse module at ${location}`,
    ...
    return { parser: language, bytes: transformedBytes, record: finalRecord };
  };
