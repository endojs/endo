...
const { sync: syncParsers, async: asyncParsers } = createParsers({
  ...
  visitorFactories: [/* (location, specifier) => VisitorPass */],
  ...
  mts: {/* per-language overrides for ESM TypeScript (experimental) */},
...
const { sync } = createParsers({
  ...
  mts: {/* visitorFactories, babelParserOptions, etc. */},
});
...
- Node.js may emit an `ExperimentalWarning`.
...
> [!TIP]
>
> Use [`erasableSyntaxOnly`](https://www.typescriptlang.org/tsconfig/#erasableSyntaxOnly) to have TypeScript enforce the strip-only subset at compile time.
...
onModuleComplete: ({
  visitorResults: [isThisDataA, maybeItsDataB],
  language,
}) => {
  // ...
};
...
Babel's parser can recover from _some_ parsing errors. By default, `@endo/parser-pipeline` will throw an `AggregateError` if any "Babel-recoverable" errors are encountered when parsing a module.

> [!NOTE]
>
> A "Babel-recoverable" parsing error generally indicates a module contains a syntax error, and thus will throw at runtime.

If you'd like to handle these errors yourself, you can provide the optional `onParseError` callback, which will be called at most once per module with a nonempty array of parse errors, and the original module location (as a string `file://` URL):

```js
onParseError: (errors, location) => {
  console.error(`Failed to parse module at ${location}`);
