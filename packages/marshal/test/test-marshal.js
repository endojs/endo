// @ts-check

import { test } from './prepare-test-env-ava.js';

import { passStyleOf } from '../src/passStyleOf.js';

import { makeMarshal } from '../src/marshal.js';
import { makeTagged } from '../src/makeTagged.js';

const { details: X } = assert;
const { freeze, isFrozen, create, prototype: objectPrototype } = Object;

// this only includes the tests that do not use liveSlots

/**
 * A list of `[plain, encoding]` pairs, where plain serializes to the
 * stringification of `encoding`, which unserializes to something deepEqual
 * to `plain`.
 */
export const roundTripPairs = harden([
  // Simple JSON data encodes as itself
  [
    [1, 2],
    [1, 2],
  ],
  [{ foo: 1 }, { foo: 1 }],
  [{}, {}],
  [
    { a: 1, b: 2 },
    { a: 1, b: 2 },
  ],
  [
    { a: 1, b: { c: 3 } },
    { a: 1, b: { c: 3 } },
  ],
  [true, true],
  [1, 1],
  ['abc', 'abc'],
  [null, null],

  // Scalars not represented in JSON
  [undefined, { '@qclass': 'undefined' }],
  [NaN, { '@qclass': 'NaN' }],
  [Infinity, { '@qclass': 'Infinity' }],
  [-Infinity, { '@qclass': '-Infinity' }],
  [4n, { '@qclass': 'bigint', digits: '4' }],
  // Does not fit into a number
  [9007199254740993n, { '@qclass': 'bigint', digits: '9007199254740993' }],

  // Well known symbols
  [Symbol.asyncIterator, { '@qclass': 'symbol', name: '@@asyncIterator' }],
  [Symbol.match, { '@qclass': 'symbol', name: '@@match' }],
  // Registered symbols
  [Symbol.for('foo'), { '@qclass': 'symbol', name: 'foo' }],
  // Registered symbol hilbert hotel
  [Symbol.for('@@foo'), { '@qclass': 'symbol', name: '@@@@foo' }],

  // Normal json reviver cannot make properties with undefined values
  [[undefined], [{ '@qclass': 'undefined' }]],
  [{ foo: undefined }, { foo: { '@qclass': 'undefined' } }],

  // tagged
  [
    makeTagged('x', 8),
    {
      '@qclass': 'tagged',
      tag: 'x',
      payload: 8,
    },
  ],
  [
    makeTagged('x', undefined),
    {
      '@qclass': 'tagged',
      tag: 'x',
      payload: { '@qclass': 'undefined' },
    },
  ],

  // errors
  [
    Error(),
    {
      '@qclass': 'error',
      message: '',
      name: 'Error',
    },
  ],
  [
    ReferenceError('msg'),
    {
      '@qclass': 'error',
      message: 'msg',
      name: 'ReferenceError',
    },
  ],

  // Hilbert hotel
  [
    { '@qclass': 8 },
    {
      '@qclass': 'hilbert',
      original: 8,
    },
  ],
  [
    { '@qclass': '@qclass' },
    {
      '@qclass': 'hilbert',
      original: '@qclass',
    },
  ],
  [
    { '@qclass': { '@qclass': 8 } },
    {
      '@qclass': 'hilbert',
      original: {
        '@qclass': 'hilbert',
        original: 8,
      },
    },
  ],
  [
    {
      '@qclass': {
        '@qclass': 8,
        foo: 'foo1',
      },
      bar: { '@qclass': undefined },
    },
    {
      '@qclass': 'hilbert',
      original: {
        '@qclass': 'hilbert',
        original: 8,
        rest: { foo: 'foo1' },
      },
      rest: {
        bar: {
          '@qclass': 'hilbert',
          original: { '@qclass': 'undefined' },
        },
      },
    },
  ],
]);

/** @typedef {import('../src/types').Validator} Validator */

/**
 * @param {import('ava').ExecutionContext<unknown>} t
 * @param {(validator?: Validator) => unknown} construct
 * @param {string} name
 * @param {(value: any, name: string) => void} checkExpected
 * @param {Validator} [failingValidator]
 * @param {any} [expectedError]
 */
const validate = (
  t,
  name,
  construct,
  checkExpected,
  failingValidator,
  expectedError,
) => {
  checkExpected(construct(), `${name} no validation`);

  const permissiveValidator = (_key, _value) => 'validate ignored';
  checkExpected(construct(permissiveValidator), `${name} permissive validator`);

  if (failingValidator) {
    t.throws(
      () => construct(failingValidator),
      expectedError,
      `${name} failing validator`,
    );
  }
};

test('serialize unserialize round trip pairs', t => {
  const { serialize, unserialize } = makeMarshal(undefined, undefined, {
    // TODO errorTagging will only be recognized once we merge with PR #2437
    // We're turning it off only for the round trip test, not in general.
    errorTagging: 'off',
  });
  for (const [plain, encoded] of roundTripPairs) {
    const { body } = serialize(plain);
    const encoding = JSON.stringify(encoded);
    t.is(body, encoding);
    validate(
      t,
      'round trip',
      makeValidator => unserialize({ body, slots: [] }, makeValidator),
      (decoding, name) => {
        t.deepEqual(decoding, plain, `${name} round trip`);
        t.assert(isFrozen(decoding), `${name} is frozen`);
      },
    );
  }
});

test('serialize static data', t => {
  const m = makeMarshal();
  const ser = val => m.serialize(val);
  t.throws(() => ser([1, 2]), {
    message: /Cannot pass non-frozen objects like/,
  });
  // -0 serialized as 0
  t.deepEqual(ser(0), { body: '0', slots: [] });
  t.deepEqual(ser(-0), { body: '0', slots: [] });
  t.deepEqual(ser(-0), ser(0));
  // unregistered symbols
  t.throws(() => ser(Symbol('sym2')), {
    // An anonymous symbol is not Passable
    message: /Only registered symbols or well-known symbols are passable:/,
  });

  const cd = ser(harden([1, 2]));
  t.is(isFrozen(cd), true);
  t.is(isFrozen(cd.slots), true);
});

test('unserialize static data', t => {
  const m = makeMarshal();
  const uns = body => m.unserialize({ body, slots: [] });

  // should be frozen
  const arr = uns('[1,2]');
  t.truthy(isFrozen(arr));
  const a = uns('{"b":{"c":{"d": []}}}');
  t.truthy(isFrozen(a));
  t.truthy(isFrozen(a.b));
  t.truthy(isFrozen(a.b.c));
  t.truthy(isFrozen(a.b.c.d));
});

test('serialize errors', t => {
  const ser = (val, makeValidator) => {
    const m = makeMarshal();
    return m.serialize(val, makeValidator);
  };

  validate(
    t,
    'serialize error',
    v => ser(harden(Error()), v),
    (encoding, name) =>
      t.deepEqual(
        encoding,
        {
          body:
            '{"@qclass":"error","errorId":"error:anon-marshal#10001","message":"","name":"Error"}',
          slots: [],
        },
        name,
      ),
  );

  validate(
    t,
    'reference error',
    v => ser(harden(ReferenceError('msg')), v),
    (encoding, name) =>
      t.deepEqual(
        encoding,
        {
          body:
            '{"@qclass":"error","errorId":"error:anon-marshal#10001","message":"msg","name":"ReferenceError"}',
          slots: [],
        },
        name,
      ),
  );

  // TODO Once https://github.com/Agoric/SES-shim/issues/579 is merged
  // do a golden of the error notes generated by the following

  // Extra properties
  const errExtra = Error('has extra properties');
  // @ts-ignore Check dynamic consequences of type violation
  errExtra.foo = [];
  freeze(errExtra);
  t.assert(isFrozen(errExtra));
  // @ts-ignore Check dynamic consequences of type violation
  t.falsy(isFrozen(errExtra.foo));
  validate(
    t,
    'extra error props',
    v => ser(errExtra, v),
    (encoding, name) =>
      t.deepEqual(
        encoding,
        {
          body:
            '{"@qclass":"error","errorId":"error:anon-marshal#10001","message":"has extra properties","name":"Error"}',
          slots: [],
        },
        name,
      ),
  );
  // @ts-ignore Check dynamic consequences of type violation
  t.falsy(isFrozen(errExtra.foo));

  // Bad prototype and bad "message" property
  const nonErrorProto1 = { __proto__: Error.prototype, name: 'included' };
  const nonError1 = { __proto__: nonErrorProto1, message: [] };
  validate(
    t,
    'non-error proto',
    v => ser(harden(nonError1), v),
    (encoding, name) =>
      t.deepEqual(
        encoding,
        {
          body:
            '{"@qclass":"error","errorId":"error:anon-marshal#10001","message":"","name":"included"}',
          slots: [],
        },
        name,
      ),
  );
});

test('unserialize errors', t => {
  const m = makeMarshal();
  const uns = (body, makeValidator) =>
    m.unserialize({ body, slots: [] }, makeValidator);

  validate(
    t,
    'reference error',
    v => uns('{"@qclass":"error","message":"msg","name":"ReferenceError"}', v),
    (em1, name) => {
      t.truthy(em1 instanceof ReferenceError, `${name} is ReferenceError`);
      t.is(em1.message, 'msg', `${name} message`);
      t.truthy(isFrozen(em1), `${name} is frozen`);
    },
  );

  validate(
    t,
    'type error',
    v => uns('{"@qclass":"error","message":"msg2","name":"TypeError"}', v),
    (em2, name) => {
      t.truthy(em2 instanceof TypeError, `${name} is TypeError`);
      t.is(em2.message, 'msg2', `${name} message`);
    },
  );

  validate(
    t,
    'unknown error',
    v => uns('{"@qclass":"error","message":"msg3","name":"Unknown"}', v),
    (em3, name) => {
      t.truthy(em3 instanceof Error, `${name} is Error`);
      t.is(em3.message, 'msg3', `${name} message`);
    },
  );
});

test('passStyleOf null is "null"', t => {
  t.assert(passStyleOf(null), 'null');
});

test('mal-formed @qclass', t => {
  const m = makeMarshal();
  const uns = body => m.unserialize({ body, slots: [] });
  t.throws(() => uns('{"@qclass": 0}'), { message: /invalid qclass/ });
});

test('records', t => {
  function convertValToSlot(_val) {
    return 'slot';
  }
  const fauxPresence = harden({});
  function convertSlotToVal(_slot) {
    return fauxPresence;
  }
  const { serialize: ser, unserialize: unser } = makeMarshal(
    convertValToSlot,
    convertSlotToVal,
  );

  const emptyData = { body: JSON.stringify({}), slots: [] };

  function build(...opts) {
    const props = {};
    for (const opt of opts) {
      if (opt === 'enumStringData') {
        props.key1 = { enumerable: true, value: 'data' };
      } else if (opt === 'enumStringGetData') {
        props.enumStringGetData = { enumerable: true, get: () => 0 };
      } else if (opt === 'enumStringGetFunc') {
        props.enumStringGetFunc = { enumerable: true, get: () => () => 0 };
      } else if (opt === 'enumStringSet') {
        props.enumStringSet = { enumerable: true, set: () => undefined };
      } else if (opt === 'nonenumStringData') {
        props.nonEnumStringData = { enumerable: false, value: 3 };
      } else {
        throw Error(`unknown option ${opt}`);
      }
    }
    // @ts-ignore Don't yet understand typing, but want dynamic test anyway
    const o = create(objectPrototype, props);
    return harden(o);
  }

  function shouldThrow(opts, message = /XXX/) {
    t.throws(() => ser(build(...opts)), { message });
  }
  const REC_NOACC = /must not be an accessor property:/;
  const REC_ONLYENUM = /must be an enumerable property:/;

  // empty objects

  // rejected because it is not hardened
  t.throws(
    () => ser({}),
    { message: /Cannot pass non-frozen objects/ },
    'non-frozen data cannot be serialized',
  );

  // harden({})
  t.deepEqual(ser(build()), emptyData);

  const key1Data = { body: JSON.stringify({ key1: 'data' }), slots: [] };

  // Serialized data should roundtrip properly
  t.deepEqual(unser(ser(harden({}))), {});
  t.deepEqual(unser(ser(harden({ key1: 'data' }))), { key1: 'data' });

  // unserialized data can be serialized again
  t.deepEqual(ser(unser(emptyData)), emptyData);
  t.deepEqual(ser(unser(key1Data)), key1Data);

  // { key: data }
  // all: pass-by-copy without warning
  t.deepEqual(ser(build('enumStringData')), {
    body: '{"key1":"data"}',
    slots: [],
  });

  // anything with getters is rejected
  shouldThrow(['enumStringGetData'], REC_NOACC);
  shouldThrow(['enumStringGetData', 'enumStringData'], REC_NOACC);
  shouldThrow(['enumStringGetFunc'], REC_NOACC);
  shouldThrow(['enumStringGetFunc', 'enumStringData'], REC_NOACC);
  shouldThrow(['enumStringSet'], REC_NOACC);
  shouldThrow(['enumStringSet', 'enumStringData'], REC_NOACC);

  // anything with non-enumerable properties is rejected
  shouldThrow(['nonenumStringData'], REC_ONLYENUM);
  shouldThrow(['nonenumStringData', 'enumStringData'], REC_ONLYENUM);
});

test('validator', t => {
  const m = makeMarshal();
  validate(
    t,
    'no bigints',
    v => m.serialize(harden({ a: { b: 'deep', bi: 3n } }), v),
    (encoded, name) => {
      t.is(
        encoded.body,
        '{"a":{"b":"deep","bi":{"@qclass":"bigint","digits":"3"}}}',
        `${name} body`,
      );
      t.deepEqual(encoded.slots, [], `${name} slots`);
    },
    (key, value) => {
      assert(typeof value !== 'bigint', X`Bigint ${value} detected`);
    },
    { message: `Bigint "[3n]" detected` },
  );

  validate(
    t,
    'key length',
    v => m.serialize(harden([{ a: 123, b: { c123456: 'hello' } }]), v),
    (encoded, name) => {
      t.is(encoded.body, '[{"a":123,"b":{"c123456":"hello"}}]', `${name} body`);
      t.deepEqual(encoded.slots, [], `${name} slots`);
    },
    (key, _val) => {
      assert(key.length <= 6, X`key ${key} too long`);
    },
    { message: `key "c123456" too long` },
  );

  validate(
    t,
    'object length',
    v => m.serialize(harden({ a: { b: { c: 'overlong' } } }), v),
    (encoded, name) => {
      t.is(encoded.body, '{"a":{"b":{"c":"overlong"}}}', `${name} body`);
      t.deepEqual(encoded.slots, [], `${name} slots`);
    },
    (key, val) => {
      assert(
        typeof val !== 'string' || val.length < 6,
        X`key ${key} value ${val} too long`,
      );
    },
    { message: `key "c" value "overlong" too long` },
  );

  let total = 0;
  validate(
    t,
    'total length',
    v => m.serialize(harden({ a: { b: { c: 'd' } } }), v),
    (encoded, name) => {
      t.is(encoded.body, '{"a":{"b":{"c":"d"}}}', `${name} body`);
      t.deepEqual(encoded.slots, [], `${name} slots`);
    },
    (key, val) => {
      if (typeof key === 'string') {
        total += key.length;
      }
      if (typeof val === 'string') {
        total += val.length;
      }
      assert(total < 4, X`total ${total} too long`);
    },
    { message: `total 4 too long` },
  );
});
