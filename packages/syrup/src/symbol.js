...
export const SyrupSymbolFor = name =>
  Symbol.for(`${SYRUP_SYMBOL_PREFIX}${name}`);
...
export const getSyrupSymbolName = symbol => {
  ...
  if (!description.startsWith(SYRUP_SYMBOL_PREFIX)) {
    throw TypeError(
      `Symbol ${String(symbol)} has a description that does not start with "${SYRUP_SYMBOL_PREFIX}", got "${description}"`,
    );
  }
