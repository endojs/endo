    ...
    const moduleVisitor = (doAnalyze, doTransform) => ({
          ...
          for (const spec of specs) {
            ...
            switch (spec.type) {
              ...
              case 'ImportSpecifier':
                // @ts-expect-error - name only on Identifier; could be StringLiteral
      ...
      ExportDefaultDeclaration(path) {
        ...
        if (doTransform) {
          ...
          // @ts-expect-error - check 'id' in decl
          if (decl.id) {
            // Just keep the same declaration and mark it as the default.
            path.replaceWithMultiple([
              // @ts-expect-error - check 'id' in declTSDeclareFunction
            ...
            replace(
              ...
              t.variableDeclaration('const', [
                t.variableDeclarator(
                  ...
                  // @ts-expect-error - typescript incompatibility
      ...
      ClassDeclaration(path) {
        ...
        // @ts-expect-error - id could be nullish
      ...
      ExportNamedDeclaration(path) {
        ...
        if (doAnalyze) {
          ...
          if (decl) {
            // @ts-expect-error - declarations only on VariableDeclaration
            const declarations = decl.declarations || [decl];
            const vids = declarations.flatMap(({ id }) =>
              collectPatternIdentifiers(path, id),
            );
          ...
          for (const spec of specs) {
            // @ts-expect-error - local and exported only on ExportSpecifier
        ...
        if (doTransform) {
          // @ts-expect-error - decl only on ExportNamedDeclaration
