...
test('CjsModuleSource requires new', t => {
  // @ts-expect-error - CjsModuleSource is a constructor
...
// A `require(...)` nested inside another call (e.g. Babel's
// `_interopRequireWildcard(require('./x'))`) is captured by the generic
// require-call branch during traversal, so it must be recorded exactly once —
// no dedicated interop handling that would double-count the specifier.
test('require nested in a call is recorded exactly once', t => {
  const ms = new CjsModuleSource(
    `var _A = _interopRequireWildcard(require('./Accordion'));`,
  );
  t.deepEqual([...ms.imports], ['./Accordion']);
});
...
test('dynamic import() with import attributes records specifier', t => {
  const ms = new CjsModuleSource(
    `import('data.json', { with: { type: 'json' } });`,
  );
  t.true(ms.__needsImport__);
  t.true(ms.imports.includes('data.json'));
});

test('dynamic import() with phase and attributes records specifier', t => {
  const ms = new CjsModuleSource(
    `import('mod.wasm', { with: { type: 'wasm' } });`,
  );
  t.true(ms.imports.includes('mod.wasm'));
});

test('require() with extra args is ignored (lexer parity)', t => {
  const ms = new CjsModuleSource(`require('foo', 'extra');`);
  t.false(ms.imports.includes('foo'));
});
...
test('module.exports object reassignment clears earlier reexports', t => {
  ...
  // object reassignment must discard it (Node only exposes `bar` here). Note
  // that the lexer only records a key when the value begins with something that
  // looks like an identifier; this also works similarly for object shorthand
  // syntax (`{ bar }` vs `{ bar: bar }`)
  const ms = new CjsModuleSource(`
    const bar = 1;
    module.exports = require('./stale');
...
test('Object.defineProperty with value descriptor', t => {
  const ms = new CjsModuleSource(`
    ...
    Object.defineProperty(exports, "__esModule", { value: true });
  `);
  // { enumerable: false, value } → rejected (lexer parity: enumerable must be true or absent)
  t.false(ms.exports.includes('namedExport'));
...
// Non-identifier export names are emitted verbatim, matching Node.js and the
// upstream `cjs-module-lexer`. Only an ES module *binding* name must be a valid
// identifier; the exported *name* may be an arbitrary string (e.g. via
// `export { local as 'weird name' }`). Only strict-reserved words are filtered
// (see below).
test('non-identifier export names are included', t => {
  const ms = new CjsModuleSource(`
    exports['not identifier'] = 'asdf';
    exports['@notidentifier'] = 'asdf';
  `);
  t.true(ms.exports.includes('not identifier'));
  t.true(ms.exports.includes('@notidentifier'));
});
