...
// Capture `Reflect.apply` once at module load, before any consumer can
// tamper with `Function.prototype.call`.  See `encodeHex` for the
// rationale.
const { apply } = Reflect;
 ...
 * indexing a lookup table.  On V8 (Node 22), this is roughly 2.5 to 3
 * times faster than the table-based decoder for ~1 MiB inputs and
 * avoids any module-scope mutable data.  On XS the polyfill is
 * unavoidably slow regardless of approach; see `test/decode.bench.js`
 * for variants and the relative trade-offs.  XS consumers should
 * always reach the native `Uint8Array.fromHex` intrinsic dispatched
 * below as soon as Moddable ships it.
 *
 * @param {string} string
