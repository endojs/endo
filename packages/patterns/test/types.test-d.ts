...
{
  const p = M.arrayOf(
    M.splitRecord({ name: M.string(), value: M.nat() }),
  );
  type T = TypeFromPattern<typeof p>;
...
// suitable for constraining Exo method implementations.
{
  ...
  type Issuer = RemotableBrand<
    {},
    { getAmountOf: (payment: any) => any }
  >;
  ...
  expectType<{ brand: Brand; issuer: Issuer }>(
    null as unknown as IssuerRecord,
  );
  ...
  const ExchangeI = M.interface('Exchange', {
    ...
    swapAsync: M.callWhen(
      M.await(M.remotable<Brand>('Brand')),
    ).returns(M.nat()),
  });
...
// M.or inside splitRecord
{
  ...
  expectType<{ value: string | bigint; label: string }>(
    null as unknown as T,
  );
}
