...
type TFTuple<T extends readonly any[]> =
 ...
 */
type TFDispatch<K extends string, Payload> =
  K extends keyof TFLeafMap<Payload>
    ? TFLeafMap<Payload>[K]
    : TFStructural<K, Payload>;

/** Phase 2: structural / recursive matcher dispatch. */
type TFStructural<K extends string, Payload> =
  K extends 'kind'
...
type TFAnd<T extends readonly any[]> =
  T extends readonly [infer H, ...infer R]
 ...
 */
type TypeFromArgGuard<G> =
  G extends { [Symbol.toStringTag]: 'guard:rawGuard' }
    ? any
...
type TFArgGuards<T extends readonly any[]> =
  T extends readonly [infer H, ...infer R]
...
type TypeFromReturnGuard<G> =
  G extends { [Symbol.toStringTag]: 'guard:rawGuard' } ? any : TypeFromPattern<G>;
 ...
 * ```
 */
export type TypeFromMethodGuard<G> =
  G extends {
    payload: {
      callKind: infer CK;
