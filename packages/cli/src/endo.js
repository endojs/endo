...
export const main = async rawArgs => {
  ...
  program
    ...
    .action(async (outboxName, description, cmd) => {
      const { name: resultName, inbox: inboxNames, wait } = cmd.opts();
      const { getBootstrap } = await provideEndoClient(
        'cli',
        sockPath,
        cancelled,
      ...
      try {
        ...
        const resultP = E(outboxP).request(description, resultName);
  ...
  program
    ...
    .action(async (importPath, outboxName, cmd) => {
      const { name: resultName, worker: workerName, inboxNames } = cmd.opts();
      const { getBootstrap } = await provideEndoClient(
        'cli',
        sockPath,
        cancelled,
