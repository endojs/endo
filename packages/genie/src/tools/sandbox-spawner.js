 ...
 * contract.
 *
 * A real `FarRef<SandboxHandle>` returned from
 * `E(sandboxFactory).make(...)` satisfies this shape directly: every
 * method below is invoked through `E(handle).foo()`, which transparently
 * unwraps the FarRef brand and the Promise-of-Promise return types.
 * Callers should hand the FarRef in directly (with a single
 * `/** @type {SandboxHandleLike} *\/` cast at the call site if the
 * JSDoc checker complains) rather than re-wrapping each method in a
 * local function — such wraps are no-ops that obscure the seam and
 * have been removed once already; do not reintroduce them "to make the
 * types align".
 *
