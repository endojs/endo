...
/** @import { IntervalTickMessage, IntervalHandle, IntervalControlFacet, IntervalSchedulerFacet } from './src/interval/types.js' */
...
export const make = (guestPowers, _context) => {
  ...
  const processHeartbeat = async (
    ...
    // TODO we need to get `tick` as a message given name and formula...
    // TODO so that tick needs to have somehow been persisted as a pet name?
    // TODO idk how this works, computer? enhance?
    // TODO we need to end up with IntervalTickMessage from the genie interval package here
    ...
    const onTick = tick => {
      switch (tick.label) {
        case 'heartbeat': {
          ...
          console.info(`[genie:${agentName}] Sending HEARTBEAT message:`, {
            tickNumber,
          ...
          E(agentGuest)
            ...
            .catch(err => {
              ...
              // Resolve the tick anyway to prevent the scheduler from stalling.
              tick.tickResponse.resolve();
            });
        }; break;
   ...
   */
    ...
    const drainPendingHeartbeats = async () => {
      ...
      for (const m of allMessages) {
        ...
        const { strings: [head = ''] } = m;
        if (head.trim().toLowerCase().startsWith('/heartbeat')) {
          extra.push(m);
        }
      }
      return extra;
