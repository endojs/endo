...
export const outlinerComponent = async (
  ...
  const buildReactPill = (targetKey, emoji, members, depth) => {
    ...
    $wrapper.className = 'outliner-react-pill-wrapper';

    const $pill = document.createElement('button');
    $pill.className = 'outliner-react-pill';
    const myId = ownMemberId || '';
    if (members.has(myId)) {
      $pill.classList.add('outliner-react-own');
    }
    ...
    if (depth < MAX_REACT_DEPTH && reactMsgKeys.length > 0) {
      ...
      if (subReacts.size > 0) {
        ...
        $sub.className = 'outliner-react-sub';
        for (const [subEmoji, subMembers] of subReacts) {
          $sub.appendChild(
  ...
  /**
   * Render react pills for a given message key.
   * Creates or updates a `.outliner-reacts` container on the node row.
   * @param {string} key
   */
  const renderReacts = key => {
    ...
    let $reactsContainer = els.$row.querySelector('.outliner-reacts');

    if (!reacts || reacts.size === 0) {
      if ($reactsContainer) $reactsContainer.remove();
      return;
    }

    if (!$reactsContainer) {
      $reactsContainer = document.createElement('span');
      $reactsContainer.className = 'outliner-reacts';
      els.$row.appendChild($reactsContainer);
    }
    $reactsContainer.innerHTML = '';

    for (const [emoji, members] of reacts) {
      $reactsContainer.appendChild(buildReactPill(key, emoji, members, 0));
    }
  };
  ...
  /**
   * Track a react and update the display.
   * @param {string} parentKey
   * @param {string} emoji
   * @param {string} memberId
   * @param {string} [msgKey]
   */
  const addReact = (parentKey, emoji, memberId, msgKey) => {
    trackReact(parentKey, emoji, memberId, msgKey);
    renderReacts(getRootMessageKey(parentKey));
  };

  /**
   * Remove a react and update the display.
   * @param {string} parentKey
   * @param {string} emoji
   * @param {string} memberId
   */
  const removeReact = (parentKey, emoji, memberId) => {
    untrackReact(parentKey, emoji, memberId);
    renderReacts(getRootMessageKey(parentKey));
  };
   ...
   * Uses Intl.Segmenter where available for accurate grapheme iteration.
   ...
   * Includes recommended reacts, an "Add react" button for the full picker,
   * and a search bar.
  ...
  const showReactPicker = ($anchor, key) => {
    ...
    const $existing = document.querySelector('.outliner-react-picker');
    if ($existing) $existing.remove();

    const $picker = document.createElement('div');
    $picker.className = 'outliner-react-picker';
    ...
    const renderGrid = query => {
      ...
      for (const [category, emojis] of EMOJI_CATEGORIES) {
        const matches = q
          ? emojis.filter(e => {
              // Match against category name or the emoji character itself
              if (category.toLowerCase().includes(q)) return true;
              // Check search index for any name substring match
  ...
  const createCommittedNode = (
    ...
    // Reply button + three-dot menu (visible on hover)
    {
      ...
      // React button
      {
        ...
        $reactBtn.className = 'outliner-react-button';
        $reactBtn.title = 'React';
        $reactBtn.type = 'button';
        $reactBtn.textContent = '\uD83D\uDE00';
        $reactBtn.addEventListener('click', e => {
          e.stopPropagation();
          showReactPicker($reactBtn, key);
        });
  ...
  const handleIncremental = (message, msgKey) => {
    ...
    } else if (message.replyType === 'react') {
      const targetKey = message.replyTo;
      if (!targetKey) return;
      const emoji = (message.strings[0] || '').trim();
      if (!emoji) return;
      addReact(targetKey, emoji, message.memberId || '', msgKey);
    } else if (message.replyType === 'redact-react') {
      const targetKey = message.replyTo;
      if (!targetKey) return;
      const emoji = (message.strings[0] || '').trim();
      if (!emoji) return;
      removeReact(targetKey, emoji, message.memberId || '');
    }
  };
  ...
  for await (const message of messageIterator) {
    ...
    if (batchTimer) {
      ...
      batchTimer = setTimeout(() => {
        ...
        for (const key of reactsForKey.keys()) {
          renderReacts(key);
        }
        scrollToBottom();
        initialLoadComplete = true;
