...
const main = async () => {
  ...
  // Connect to the system Endo daemon
  let endoBootstrap;
  try {
    ...
    endoBootstrap = getBootstrap();

    // Handle daemon disconnection
    closed.catch(error => {
      console.error('[Gateway] Daemon connection closed:', error.message);
      process.exit(1);
    });
  } catch (error) {
    console.error('[Gateway] Failed to connect to Endo daemon:', error.message);
  ...
  const gateway = await E(endoBootstrap).gateway();
  const host = await E(endoBootstrap).host();

  // Get the AGENT identifier for the host
  const endoId = await E(host).identify('AGENT');
  ...
  wss.on('connection', (socket, req) => {
    ...
    const { dispatch, getBootstrap, abort: abortCapTP } = captp;
