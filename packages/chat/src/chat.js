...
const inboxComponent = async ($parent, $end, powers) => {
  ...
  for await (const message of makeRefIterator(E(powers).followMessages())) {
    ...
    const { number: rawNumber, from: fromId, to: toId, date, dismissed } = message;
    ...
    if (message.type === 'request') {
      ...
      if ($senderChip) {
        const $senderLine = document.createElement('p');
        $senderLine.appendChild($senderChip);
        $senderLine.appendChild(document.createTextNode(' proposed definition:'));
        $body.appendChild($senderLine);
      }
      ...
      if (slotEntries.length > 0) {
        ...
        for (const [slotName, slotInfo] of slotEntries) {
          ...
          $label.title = /** @type {{ label: string }} */ (slotInfo).label || '';
          $row.appendChild($label);

          const $desc = document.createElement('span');
          $desc.className = 'slot-description';
          $desc.textContent = /** @type {{ label: string }} */ (slotInfo).label || '';
        ...
        $rejectBtn.innerText = 'Reject';
        $rejectBtn.onclick = () => {
          E(powers).reject(number, 'Definition rejected').catch(window.reportError);
        };
        $controls.appendChild($rejectBtn);
