...
const compartmentMapForNodeModules_ = async (
  ...
  /**
   * All package locations from the additional packages; used with
   * {@link retainAdditionalModules}.  We can safely ignore any that were
   * already found.
   */
  const additionalPackageLocations = additionalPackageDetails.reduce(
    (acc, details) => {
      if (!basePackageLocations.has(details.packageLocation)) {
        acc.set(details.packageLocation, details);
      }
      return acc;
    },
    /** @type {Map<FileUrlString, AdditionalPackageDetails>} */ (new Map()),
  );
  ...
  const graphsByAdditionalLocation = new Map(
    [...additionalPackageLocations.keys()].map(additionalPackageLocation => [
  ...
  retainAdditionalModules(
    compartmentMap,
    additionalPackageLocations.keys(),
