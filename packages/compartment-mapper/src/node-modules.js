...
import { makeDefaultHookConfiguration, makeHookExecutor } from './hook.js';
        ...
        try {
          for (const label of packageDependenciesHookResult.dependencies) {
            ...
            if (name && packageLocation) {
              ...
              log(
                `WARNING: packageDependencies hook returned unknown package with label ${label}`,
              );
            }
          }
...
export const compartmentMapForNodeModules_ = async (
  ...
  executeHook('canonicalNames', {
    canonicalNames: new Set([...canonicalNameMap.keys()]),
    log,
  });
