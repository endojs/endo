...
const makeLoadCompartments = (
  ...
  const forceLoadCompartments = async compartments => {
    const compartmentsToLoad = forceLoad.reduce((acc, canonicalName) => {
      ...
      if (!compartmentDescriptor) {
        throw new ReferenceError(
          `Failed attempting to force-load unknown compartment ${q(canonicalName)}`,
        );
      }
      ...
      if (keys(compartmentSources).length) {
        log(
          `Refusing to force-load Compartment ${q(canonicalName)}; already loaded`,
        );
        return acc;
      }
      ...
      if (!compartmentOwnModuleDescriptor?.module) {
        throw new Error(`Cannot determine entry point of ${q(canonicalName)}`);
      }
      acc.push([
        canonicalName,
        compartment,
