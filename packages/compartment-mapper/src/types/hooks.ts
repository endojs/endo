...
import type { FinalGraph, PackageDescriptor } from './node-modules.js';
...
export type MapNodeModulesHooks = {
  ...
  /**
   * Hook executed after a {@link PackageCompartmentDescriptor} is created.
   */
  compartmentDescriptor: HookFn<{
    compartmentDescriptor: PackageCompartmentDescriptor;
  }>;
};

/**
 * Configuration for possible hooks into import location operations.
 */
export type ImportLocationHooks = {
  /**
   * Hook executed when processing a module source.
   */
  moduleSource: HookFn<{ source: ModuleSource }>;
};

/**
 * Configuration for hooks available in the import hook maker.
 */
export type MakeImportHookMakerHooks = ImportLocationHooks;

/**
 * Configuration for hooks available during the linking phase.
 */
export type LinkHooks = MakeImportHookMakerHooks;

/**
 * Configuration for hooks available during load location operations.
 */
...
export interface HookDefinitions {
  ...
  importLocation: ImportLocationHooks;
