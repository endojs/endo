...
import { makeHookExecutor, makeDefaultHookConfiguration } from '../src/hook.js';
...
const testCanonicalNames = new Set([
  'package-a',
  'package-b',
  '@scope/package-c',
  'package-a>package-b',
  'package-a>@scope/package-c',
  'some-package>package-c', // This will match the suggestion logic
]);
...
test('makeDefaultHookConfiguration - mapNodeModules - with policy', t => {
  ...
  t.true('canonicalNames' in config, 'should have canonicalNames hook');
  t.true(
    Array.isArray(config.packageDependencies),
    'packageDependencies should be an array',
  );
  t.true(
    Array.isArray(config.canonicalNames),
    'canonicalNames should be an array',
  );
  t.is(
  ...
  t.is(config.canonicalNames?.length, 1, 'canonicalNames should have one hook');
});
...
test('makeDefaultHookConfiguration - named configuration', t => {
  ...
  t.true(
    'canonicalNames' in config,
    'should have canonicalNames hook directly',
  );
});
...
test('makeDefaultHookConfiguration - canonicalNames hook validation', t => {
  const config = makeDefaultHookConfiguration('mapNodeModules', {
    policy: {
      resources: {
        'non-existent-package': { packages: {} },
        'package-a>non-existent-nested': { packages: {} },
      },
    },
  });

  const canonicalNamesHook = /** @type {AnyHook[]} */ (config.canonicalNames);

  // Should throw for non-existent resources - hook is now an array
  const error = t.throws(() => {
    canonicalNamesHook[0]({
      canonicalNames: testCanonicalNames,
      log: () => {},
    });
  });

  t.true(
    error?.message.includes('Unknown resources found in policy'),
    'should validate resource existence',
  );
  t.true(
    error?.message.includes('non-existent-package'),
    'should mention invalid resource',
  );
});

test('makeDefaultHookConfiguration - mapNodeModules - canonicalNames hook with suggestions', t => {
  const config = makeDefaultHookConfiguration('mapNodeModules', {
    policy: {
      resources: {
        'package-b': { packages: {} }, // This exists, should be fine
        'package-c': { packages: {} }, // This doesn't exist, but @scope/package-c does
      },
    },
  });
  ...
  t.true(
    error?.message.includes('some-package>package-c'),
    'should suggest similar canonical name',
  );
});
