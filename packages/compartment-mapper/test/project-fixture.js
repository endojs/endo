 ...
 * @import {CompartmentMapDescriptor, LogFn, MaybeReadFn, PackageDescriptor, PackagePolicy} from '../src/types.js'
 * @import {ExecutionContext} from 'ava'
 */

const { entries, fromEntries, assign, getPrototypeOf, freeze, keys } = Object;
...
/**
 * Prepends a `CompartmentDescriptor.path` with the computed canonical name (or
 * "[Entry]" if entry compartment and "[Attenuators]" if attenuators
 * compartment).
 *
 * Returns a shallow copy of `path` with a {@link CustomInspectFunction}.
 *
 * @param {string[]|undefined} path {@link CompartmentDescriptor.path}
 * @param {string} name {@link CompartmentDescriptor.name}
 */
const stylePath = (path, name) => {
  if (path === undefined) {
    return undefined;
  }
  ...
  assign(fancyPath, {
    ...
    [inspect.custom]: (_, options, inspect) => {
      ...
      return path.length
        ? `${kind}(${stylize(path.join('>'), 'endoCanonical')}) ${inspect(path, options)}`
        : `${kind}(${stylize('Entry', 'endoConstant')})`;
    },
  });
  return fancyPath;
};
