/**
 * @import { IsCancelled } from '../src/types.js'
 */
...
test('delay fulfills with undefined after ms', async t => {
  ...
  // Wall-clock floor is intentionally loose: XS and loaded CI runners
  // routinely deliver the timer 5 to 20ms early on a nominal 50ms wait,
  // and an upper bound guards against the timer never firing at all.
  t.true(
    elapsed >= 30 && elapsed < 5000,
    `elapsed ${elapsed}ms should be in [30ms, 5000ms)`,
  );
});
...
test('delay treats parentCancelled fulfillment as error', async t => {
  // The delay contract types parentCancelled as Promise<never>, but the
  // implementation must guard against a misuse where the caller hands in
  // a promise that fulfills. Cast the fulfilling promise at the call
  // site so the runtime behavior is exercised without disabling the
  // type checker for the surrounding scope (which @ts-expect-error
  // would do and which silently flips when upstream types tighten).
  const fulfillingPromise = /** @type {Promise<never>} */ (
    /** @type {unknown} */ (Promise.resolve())
  );

  await t.throwsAsync(delay(50, fulfillingPromise), {
