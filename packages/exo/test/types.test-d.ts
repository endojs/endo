    ...
    () => ({}),
    {
      alice: {
        ping() {
          ...
          expectAssignable<{ alice: { ping: () => void }; bob: { pong: () => void } }>(
            this.facets,
          );
...
// A kit where one facet returns another facet, typed via M.remotable<G>
{
  ...
  const makeKit = defineExoClassKit(
    ...
    (initial: string) => ({ data: initial }),
    {
      ...
      admin: {
        getPublic() { return this.facets.public; },
      },
    },
  );
  const kit = makeKit('hello');
...
// TypeFromPattern resolves M.promise()'s Payload (Promise<any>) through
// TFStructural, producing Promise<Promise<any>> — a known awkwardness from
// the matcher's Payload representing the full promise type rather than the
// inner type.  In practice, use M.await(M.promise()) if you need to await
// and then check the resolved value.
{
  const mg = M.callWhen(M.promise()).returns(M.string());
  type Fn = TypeFromMethodGuard<typeof mg>;
  // arg resolves to Promise<Promise<any>> due to TFStructural wrapping Payload
  expectType<(p: Promise<Promise<any>>) => Promise<string>>(
...
// from another facet to its caller via Promise return
{
  ...
  expectAssignable<Promise<{ get: () => string }>>(null as unknown as LoadResult);
