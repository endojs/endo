...
const jsonEnd = ({ ok, file, agent, mode, lockdown, compartment }) => {
...
const main = async () => {
  ...
  if (showList) {
    ...
    const begin = jsonReport ? Function.prototype : verboseBegin;
    const end = jsonReport ? jsonEnd : terseEnd;
    for await (const test of runTests({ tacet: jsonReport, begin }, tests)) {
      end(test);
      if (!test.ok) {
        process.exitCode = 1;
      }
