...
This package exports two functions: `pathCompare` and `stringCompare`.

### `pathCompare(a?: string[], b?: string[]) => number`

Compares two arrays of strings (if provided) and returns a numeric comparison result: 0 if they are equal, -1 if `a` is "less than" `b`, and 1 if `a` is "greater than" `b`.
...
Compares two strings using UTF-16 code unit order and returns a numeric comparison result: 0 if they are equal, -1 if `a` is "less than" `b`, and 1 if `a` is "greater than" `b`.

```js
import { stringCompare } from '@endo/path-compare';

const str1 = 'foo';
const str2 = 'bar';

const result = stringCompare(str1, str2); // 1
```
