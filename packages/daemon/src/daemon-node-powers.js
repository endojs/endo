...
export const makeDaemonicPersistencePowers = (
  ...
  const provideRootKeypair = async () => {
    ...
    if (existingKeypair === undefined) {
      ...
      const privateHex = Buffer.from(keypair.privateKey).toString('hex');
      ...
      const publicKey = new Uint8Array(Buffer.from(lines[0], 'hex'));
      const privateKey = new Uint8Array(Buffer.from(lines[1], 'hex'));
