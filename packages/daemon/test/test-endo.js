...
/**
 * Calls `followChanges()`, drains the resulting iterator of all already-existing
 * names, and then returns the iterator in order to observe new changes.
 *
...
test('name changes subscription first publishes existing names', async t => {
  ...
  const existingNames = await E(host).list();
  const changesIterator = makeRefIterator(await E(host).followChanges());
  const values = await drainAsyncIterator(
    changesIterator,
