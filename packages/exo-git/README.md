...
- `makeGit({ mount, backend, lineageOf, readOnly })` — the `EndoGit` exo factory.  `backend` is any object satisfying the `GitBackend` protocol (`@endo/endo-git` provides the Node-side implementation).
...
- `@endo/endo-git` — the Node-side `NativeGitBackend` (subprocess wrapper over the installed `git` binary).
