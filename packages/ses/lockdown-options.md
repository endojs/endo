...
**Background**: The `Math` object includes a `random` function.  This is a
synchronous pseudo-random-number generator that can leak information to other
programs sharing this intrinsic.

The sequence of numbers generated by a pseudo-random-number are deterministic
so a program that knows the sequence can eventually infer all of the underlying
entropy by synchronsly observing the seqeunce of generated numbers.
Thereafter, they can observe whether another program has drawn from the random
number generator in any future event, advancing their simulation until it is in
sync with the shared generator.

Random number generation is useful and there are safe alternatives
to drawing from `Math.random`, and safe ways to inject an independent
generator into a compartment.

To allow `Math.random` in compartments after `lockdown`, the `unsafe` option
leaves the incubator realm's generator in every child compartment.

A safer alternative is to inject `Math.random` into select programs into
single-tenant compartments.

```js
new Compartment({
  Math: harden({
    ...Math,
    random: harden(Math.random),
  }),
})
```
...
**Background**: The `Date` constructor and `Date.now` provide a vector for
timing attacks and preclude determinism. Lockdown erases these behaviors in
child compartments by default (`safe`).

For cases where determinism is unnecessary, where knowing the time is
more useful than dangerous, or when the resolution of the timer is
not threatening, this can be generally disabled (`unsafe`).

For cases where specific compartments need access to time, the real
time or a mock time can be injected into a compartment as an endowment
or injected as a function argument.

## `stackFiltering` Options

**Background**: The error stacks shown by many JavaScript engines are
voluminous.
They contain many stack frames of functions in the infrastructure, that are
