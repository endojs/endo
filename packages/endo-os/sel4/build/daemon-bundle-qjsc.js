  ...
  return harden({
    [Symbol.asyncIterator]: () => {
      ...
      return harden({
        ...
        next: async () => {
          if (entries7 === void 0) {
            const filePath = path instanceof URL ? path : new URL(path, import.meta.url);
...
// src/help-text.js
var helpMap = readHelpTextFileSync(new URL("./help.md", import.meta.url));
...
// src/pet-sitter.js
var makePetSitter = (petStore, specialNames) => {
  const has = (petName) => {
  ...
  const list = () => {
    ...
    return harden_default([...specialKeys, ...petStore.list()]);
  };
...
var makeChannelMaker = ({ provide, persistValue, randomHex256 }) => {
...
var makeDaemonCore = async (powers, rootEntropy, {
    ...
    await Promise.all(
      entries7.map(async ({ id, formula }) => {
        ...
        if (assertValidName !== void 0) {
          ...
          const storedIds = petStore.list().map((petName) => petStore.identifyLocal(petName)).filter((storedId) => storedId !== void 0);
  ...
  const makers = {
      ...
      const endoBootstrap = makeExo("Endo", EndoInterface, {
        ...
        addPeerInfo: async (peerInfo) => {
          const knownPeers = (
            ...
            /** @type {unknown} */
            await provide(peersId, "pet-store")
          );
    ...
    "synced-pet-store": async (formula, _context, id, formulaNumber) => {
      ...
      return Far("SyncedPetStore", {
        storeLocator: async (petName, locator) => {
  ...
  const getPeerIdForNodeIdentifier = async (nodeNumber) => {
    ...
    const knownPeers = (
      /** @type {KnownPeersStore} */
      /** @type {unknown} */
      await provide(knownPeersId, "pet-store")
    );
...
//   makeDaemon
// };
globalThis.EndoDaemon = { makeDaemon };
